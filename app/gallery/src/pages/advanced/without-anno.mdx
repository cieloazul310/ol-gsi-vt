...
`MVTFormat` の `layers` オプションには**描写するソースレイヤー**を指定します。ということは、描写しないソースレイヤーを指定したいときは、それ以外の全てのレイヤーを入力しなければなりません。これは面倒なので、`@cieloazul310/ol-gsi-vt` では描写しないソースレイヤーを指定するための `gsiOptVtLayerExclude` という関数を用意しています。
