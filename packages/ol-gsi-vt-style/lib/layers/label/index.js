...
import { isNumber, isString, dspPosToPosition, } from '@cieloazul310/ol-gsi-vt-style-utils';
import labelOrder from './labelOrder';
export default function labelStyle(feature, resolution, { palette, zIndex, ...theme }) {
    const { ftCode, knj, dspPos, annoCtg, arrng, arrngAgl } = feature.getProperties();
    if (!isNumber(ftCode))
        throw new Error();
    if (!isNumber(annoCtg))
        throw new Error();
